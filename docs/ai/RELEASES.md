...
Planning does not authorize publication. Release requests cover local preparation; remote publication needs explicit authority. After publication, make an `ai-dev` ledger commit with the aggregate `Task-ID`, real refs/artifacts, `resolved_at`, and `done` status. Use the `Europe/Belgrade` date. This post-tag record is excluded from the next snapshot's children. After Snapshot 8, keep the minor active for feedback and do not create Snapshot 9.
...
Late assets may be attached with exact source, provenance and hashes. An authorized incomplete prerelease lists every pending platform, asset or check and remains active. Never silently replace accepted macOS bytes, move an immutable tag, or misstate source identity. Shared/gameplay changes invalidate affected acceptance. `done` waits for all required artifacts and runtime evidence; missing evidence is never inferred.
