...
A routine local report uses `ai_check.py --fast`; the commit and ordinary AI-branch push hooks own this changed-scope development gate. A fast check never substitutes for the full integration gate. Group up to 6–8 independent tasks only when their file ownership is disjoint; at the batch boundary the coordinator runs one full `python3 script/ai_check.py` plus the batch integration checks, then pushes the batch before starting another. Run reusable dependency/environment checks once per candidate, fail before expensive work when they fail, and reuse passing evidence only while inputs are unchanged. Do not add repeated broad reviews or duplicate precommit checks between successful focused checks.
