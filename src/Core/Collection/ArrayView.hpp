...
namespace core::collection {
    ...
    public:
        ...
        constexpr ArrayView(T* ptr, size_t size) noexcept : m_data(ptr, size * sizeof(T)) { }
        ...
        PURE constexpr auto&& operator[](this auto&& self, size_t idx) {
            ASSERT(idx * sizeof(T) + (sizeof(T) - 1) < self.m_data.size());
            return self.data()[idx];
        }

        PURE constexpr ArrayView subview(size_t from = 0ull, size_t count = static_cast<size_t>(-1) / sizeof(T)) const noexcept {
