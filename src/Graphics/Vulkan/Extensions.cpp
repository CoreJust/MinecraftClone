...
#define CHECK_EXTENSION(ext, name)                                                              \
            if (auto it = extensionVersions.find(#name); it != extensionVersions.end())         \
                g_supportedExtensions[static_cast<size_t>(VulkanExtension:: ext)] = Version::fromVk(it->second)
        CHECK_EXTENSION(BufferDeviceAddress, VK_KHR_buffer_device_address);
        CHECK_EXTENSION(DebugUtils,          VK_EXT_debug_utils);
        CHECK_EXTENSION(MemoryBudget,        VK_EXT_memory_budget);
