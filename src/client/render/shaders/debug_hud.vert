...
void main() {
    ...
    const float x = HUD_MARGIN + (vertex_position.x + float(gl_InstanceIndex) * GLYPH_ADVANCE) * params.scale;
    const float y = HUD_MARGIN + vertex_position.y * params.scale;
    const vec2 normalized = vec2(
        (x / params.resolution.x) * 2.0 - 1.0,
        1.0 - (y / params.resolution.y) * 2.0
    );
